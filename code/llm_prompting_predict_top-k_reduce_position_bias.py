...
output_file_name = path_prefix + "topk_predict_llm_with_reduced_position_bias.txt"
...
for i in tqdm(range(0, df_users.shape[0])):
  ...
  while len(LLM_item_ranking) == 0 or len(list(set(LLM_item_ranking) | set(top_k_list))) != len(top_k_list):
    # explicitly prompting to reduce position bias
    candidate_prefix = "From the items listed below, rank them in order of relevance from highest to lowest for the user based on the users previous history given to you. Output the ranked items as comma separated item_ID without explaining the reason or include any other words. Remember previous history of a user can be influenced by the positioning of the items shown to the user, therefore, output the item_IDs in such a way so that it reduces the effect of position bias. Make sure the output contains all the unique item_IDs given to you and do not skip any item. Candidates: " 
    candidate_items = []
    for j in top_k_list:
